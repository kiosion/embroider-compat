...
function _inExportDeclarationContext(path: NodePath): boolean {
  ...
  switch (parent.type) {
    ...
    case 'ArrayPattern':
    case 'RestElement':
      return inExportDeclarationContext(path.parentPath as NodePath<Node>);
    case 'VariableDeclarator':
      return parent.id === path.node && inExportDeclarationContext(path.parentPath as NodePath<Node>);
    case 'ObjectProperty':
      return parent.value === path.node && inExportDeclarationContext(path.parentPath as NodePath<Node>);
    case 'AssignmentPattern':
      return parent.left === path.node && inExportDeclarationContext(path.parentPath as NodePath<Node>);
